#!/usr/bin/env python3
"""
SLURM Job Submission Script for Multi-Seed Analysis

This script generates and submits SLURM jobs for each detector/dataset combination
to run the multi_seed_analysis.py script.

Usage:
    python submit_multi_seed_jobs.py [--sbatch-file FILE] [--dry-run] [--max-jobs N]
"""
...
def generate_job_name(detector, dataset):
    ...
    return f"multiseed-{clean_detector}-{clean_dataset}"
...
def submit_sbatch(sbatch_file, detector, dataset, dry_run=False):
    ...
    cmd = ['sbatch', '--job-name', job_name, str(sbatch_file), detector, dataset]
...
def main():
    parser = argparse.ArgumentParser(description='Submit SLURM jobs for multi-seed analysis')
    parser.add_argument('--sbatch-file', type=str, default='run_multi_seed.sbatch',
    ...
    if not args.dry_run and submitted_jobs:
        ...
        # Save job list
        job_list_file = script_dir / "submitted_multi_seed_jobs.txt"
